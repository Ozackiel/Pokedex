import { fetchPokemon, fetchAllPokemonNames, fetchPokemonsByType } from "./api.js";
...
let visibleCount = POKEMON_PER_PAGE;
...
// ==========================================
function renderStatRow(label, value, percent, color) {
    return `
        <div class="stat-row">
...
function openPokemonModal(pokemon) {
    if (!modalBody || !pokemonModal)
        return;
    const calcPercent = (val) => Math.min(100, Math.round((val / 255) * 100));
    modalBody.innerHTML = `
...
// ==========================================
async function updateCatalogAndRender(isLoadMore = false) {
    if (!isLoadMore) {
    ...
    if (selectedTypes.size === 0) {
        ...
        const typeLists = await Promise.all(Array.from(selectedTypes).map((type) => fetchPokemonsByType(type)));
    ...
    if (query !== "") {
        filteredList = activeCatalog.filter((p) => p.name.toLowerCase().includes(query) || p.id.toString() === query);
    }
...
typeFilterContainer.addEventListener("click", async (event) => {
    ...
    const button = target.closest(".type-btn");
    if (!button)
        return;
